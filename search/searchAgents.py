...
class CornersProblem(search.SearchProblem):
    ...
    def getSuccessors(self, state: Any):
        ...
        for action in [
            ...
            successors.append(((nextx, nexty, *isCornerVisited), action, 1))
...
def cornersHeuristic(state: Any, problem: CornersProblem):
    ...
    x, y, *isCornerVisited = state
    unvisited_corners = [
        corners[i] for i in range(len(corners)) if not isCornerVisited[i]
    ]
    ...
    for path in itertools.permutations(unvisited_corners):
        x0, y0 = x, y
        distance = 0
        for cx, cy in path:
            distance += abs(cx - x0) + abs(cy - y0)
            x0, y0 = cx, cy
...
def foodHeuristic(state: Tuple[Tuple, List[List]], problem: FoodSearchProblem):
    ...
    if len(foods) == 0:
        return 0
    left = min([food[0] for food in foods])
    right = max([food[0] for food in foods])
    down = min([food[1] for food in foods])
    up = max([food[1] for food in foods])
    height_middle = (down + up) // 2
    width_middle = (left + right) // 2
    corners = [
        (left, down),
        (left, up),
        (right, down),
    ...
    for cx, cy in corners:
        food_filtered.add(
            min(
                foods,
                key=lambda co: abs(co[0] - cx) + abs(co[1] - cy),
            )
        )
    food_list = list(food_filtered)
    min_distance = float("inf")

    import itertools

    for path in itertools.permutations(food_list):
        x0, y0 = position
        distance = 0
        for cx, cy in path:
            distance += abs(cx - x0) + abs(cy - y0)
